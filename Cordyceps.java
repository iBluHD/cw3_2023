...
public class Cordyceps extends Lifeform {
    ...
    public static float INFECTION_PROBABILITY = 0.3f;
    ...
    public void act(Cell cell) {
        // Spread the parasite(well, technically its actually a fungi, not a parasite
        // but whatever)

        List<Cell> neighbours = cell.getField().getNeighbours(cell.getLocation(), true, null);
        for (Cell neighbour : neighbours) {
            ...
            if (neighbourLifeform.getInfectionType() != "cordyceps") {
                if (rand.nextFloat() <= INFECTION_PROBABILITY) {
                    // neighbouring cell lifeform is not a cordycep, and passes probability check to
                    // become a cordycep.
